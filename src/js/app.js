(function() {
	...
	var handleInstagramResponse = window.handleInstagramResponse = function(response) {
		...
		_.forEach(response.data, function(gram, index) {
			if (gram.location && _.isNumber(gram.location.latitude) && _.isNumber(gram.location.longitude)) {
				...
				var marker = L.marker([gram.location.latitude, gram.location.longitude], { icon: markerIcon }).addTo(map);
	...
	var handleTwitterResponse = function(response) {
		...
		_.forEach(geoTweets, function(status) {
			...
			// console.log(status);
			var marker = L.marker([status.geo.coordinates[0], status.geo.coordinates[1]], { icon: markerIcon }).addTo(map);
			...
			marker.popupContent = popupTemplate({
				postUrl: twitterLinkTemplate({ username: status.user.screen_name, statusId: status.id_str }),
				imgUrl: imageUrl,
	...
	var handleOMSClick = function(marker) {
		// var markerPt = map.latLngToLayerPoint(marker.getLatLng());

		popup.setContent(marker.popupContent);
		// popup.setLatLng(map.layerPointToLatLng([markerPt.x, markerPt.y - 60]));
	...
	$(document).on('ready', function() {
		...
		$('form#tag-search').on('submit', function(event) {
			...
			if (tag.length) {
				...
				$.getJSON('/api/twitter', $.param({ q: tag, count: 100 }), handleTwitterResponse);
			}
