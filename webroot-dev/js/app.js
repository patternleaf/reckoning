(function() {
    ...
    var setupMap = function() {
        ...
        gLineFactory = d3.svg.line().interpolate("linear").x(function(d) {
    ...
    var updatePanes = function() {
        if (gTimelineEvents.length) {
            var bounds = gPathFactory.bounds({
                ...
                geometry: {
                    type: "LineString",
                    coordinates: _.map(gTimelineEvents, function(d) {
                        return d.coordinates;
                    })
                },
                properties: {}
            }), topLeft = bounds[0], bottomRight = bounds[1];
            gSVGOverlayPane.attr("width", bottomRight[0] - topLeft[0] + 120).attr("height", bottomRight[1] - topLeft[1] + 120).style("left", topLeft[0] - 50 + "px").style("top", topLeft[1] - 50 + "px");
            gTimelineGroup.selectAll(".event-path").remove();
            gTimelineGroup.append("path").datum({
                events: gTimelineEvents
            }).attr("class", "event-path").attr("d", function(d) {
    ...
    var handleTwitterUserTimelineResponse = function(response) {
        var geoTweets = _.filter(response, function(status) {
            return !!status.geo;
        });
        gTimelineEvents = [];
    ...
    $(document).on("ready", function() {
        ...
        $("form#tag-search").on("submit", function(event) {
            ...
            if (tag.length) {
                ...
                instagramTunnel.attr("src", gInstagramRequestTemplate({
                    tag: tag
                }));
